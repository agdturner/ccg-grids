 ...
 * more optimally.
 */
public abstract class Grids_AbstractStatisticsBigDecimal extends Grids_Object
        implements Serializable, Grids_InterfaceStatisticsBigDecimal {
    ...
    /**
     * For controlling how many decimal places the arithmetic mean is calculated
     * to.
     */
    public int NumberOfDecimalPlacesForArithmeticMean;

    /**
     * For controlling how many decimal places the standard deviation is
     * calculated to.
     */
    public int NumberOfDecimalPlacesForStandardDeviation;

    protected Grids_AbstractStatisticsBigDecimal() {
    }
 ...
 used to update this.
    ...
    protected void update(
        ...
        Sum = statistics.Sum;
        Min = statistics.Min;
        MinCount = statistics.MinCount;
        Max = statistics.Max;
        MaxCount = statistics.MaxCount;
    }
    ...
    protected void update(BigDecimal valueBigDecimal) {
        setNonNoDataValueCount(NonNoDataValueCount.add(BigInteger.ONE));
        setSum(Sum.add(valueBigDecimal));
        if (valueBigDecimal.compareTo(Min) == -1) {
            setMinCount(BigInteger.ONE);
            setMin(valueBigDecimal);
        } else {
            if (valueBigDecimal.compareTo(Min) == 0) {
                setMinCount(getMinCount().add(BigInteger.ONE));
            }
        }
        if (valueBigDecimal.compareTo(Max) == 1) {
            setMaxCount(BigInteger.ONE);
            setMax(valueBigDecimal);
        } else {
            if (valueBigDecimal.compareTo(Max) == 0) {
                setMaxCount(getMaxCount().add(BigInteger.ONE));
            }
        }
    }
    ...
    public String toString() {
        ...
        result += "(NonNoDataValueCountBigInteger("
                + NonNoDataValueCount.toString() + " ),"
                + "Max(" + Max.toString() + "),"
                + "Min(" + Min.toString() + "),"
                + "MaxCount(" + getMaxCount().toString() + " ),"
                + "MinCount(" + getMinCount().toString() + " ),"
                + "Sum(" + Sum.toString() + "))";
        return result;
    }
     ...
     */
    @Override
    public final BigInteger getNonNoDataValueCount(boolean handleOutOfMemoryError) {
        try {
            ...
            if (handleOutOfMemoryError) {
                ...
                ge.initMemoryReserve(false);
                return getNonNoDataValueCount(handleOutOfMemoryError);
            } else {
                throw e;
            }
        }
     ...
     * @return
     */
    public final BigInteger getNonZeroAndNonNoDataValueCount(
            boolean handleOutOfMemoryError) {
        try {
            BigInteger result = getNonZeroAndNonNoDataValueCount();
            ge.tryToEnsureThereIsEnoughMemoryToContinue(handleOutOfMemoryError);
            return result;
        } catch (OutOfMemoryError e) {
            if (handleOutOfMemoryError) {
                ...
                ge.initMemoryReserve(false);
                return getNonZeroAndNonNoDataValueCount(handleOutOfMemoryError);
            } else {
                throw e;
            }
        }
     ...
     * BigInteger.
     */
    protected BigInteger getNonZeroAndNonNoDataValueCount() {
        BigInteger result;
...
//            }
    ...
    /**
     * For returning the maximum of all data values as a BigDecimal.
     *
     * @param update If true then an update of the statistics is made.
     * @return
     */
...
//    }
     ...
     * @return
     */
    protected abstract BigDecimal getArithmeticMean();
    ...
    public Object[] getQuantileClassMap(
        ...
        if (Grid instanceof Grids_GridDouble) {
            ...
            nonZeroAndNonNoDataValueCount = getNonZeroAndNonNoDataValueCount(
