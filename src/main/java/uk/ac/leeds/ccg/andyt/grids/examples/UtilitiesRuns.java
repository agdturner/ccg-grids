...
public class UtilitiesRuns extends Grid2DSquareCellProcessor implements Runnable {
    ...
    public static void main(String[] args) {
        try {
            UtilitiesRuns u = new UtilitiesRuns();
        u.run();
        } catch ( Exception e ) {
            e.printStackTrace();
    ...
    public void kernelVolume() {
        ...
        for ( int bw = 100; bw < 1000; bw += 100 ) {
            ...
            int precision = 1000;
            System.out.println( Kernel.getKernelVolume( bandwidth, precision, weightIntersect, weightFactor ) );
        }
    }
...
//        pw.close();
    ...
    throws Exception {
        ...
        Object[] result = Utilities.densityPlot( xGrid, yGrid, divisions, grid2DSquareCellDoubleFactory );
        double[] stdevy = ( double[] ) result[ 0 ];
        double[] meany = ( double[] ) result[ 1 ];
        double[] numy = ( double[] ) result[ 2 ];
        Grid2DSquareCellDouble densityPlotGrid = ( Grid2DSquareCellDouble ) result[ 3 ];
...
//        //System.out.println( yGrid.toString() );
//        System.out.println("Processing...");
//        Utilities.toGainsChartCSV( yGrid, xGrid, divisions, new File( outDataDirectory + yFilename + xFilename + divisions + "GainsChart.csv" ) );
//        System.out.println("Finalising...");
    }
}
