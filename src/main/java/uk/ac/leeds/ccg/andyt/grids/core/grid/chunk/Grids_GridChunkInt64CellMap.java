...
/**
 * Grids_AbstractGridChunkInt extension that stores cell values in a
 * TIntLongHashMap. The maximum size of this Grids_AbstractGridChunkInt is 64
 * cells. TODO: Experiment for what type of data and purpose this is useful.
 */
public class Grids_GridChunkInt64CellMap
     ...
     * @param chunkID The ID this will have.
     */
    protected Grids_GridChunkInt64CellMap(
            Grids_GridInt g,
        ...
        initGrid(g);
        long nChunkCells
                = (long) g.getChunkNRows(ChunkID, handleOutOfMemoryError)
                * (long) g.getChunkNCols(handleOutOfMemoryError);
        if (nChunkCells <= 64) {
            initData();
        } else {
            throw new Error(
                    "Error in " + this.getClass().getName() + ": " + nChunkCells + " > 64");
     ...
     *
     *
     * @param chunk The Grids_AbstractGridChunkInt this is constructed from.
     * @param chunkID The ID this will have.
     */
    protected Grids_GridChunkInt64CellMap(
    ...
    /**
     * Returns all the values (not including noDataValues) in row major order as
     * a double[].
     *
     * @return
     */
    ...
    int setCell(
        ...
        int position
                = (chunkCellRowIndex * g.getChunkNCols(handleOutOfMemoryError))
                + chunkCellColIndex;
    ...
    protected long powerOf2(
            int value) {
        Grids_GridInt grid2DSquareCellInt = this.getGrid();
    ...
    /**
     * For returning the Arithmetic Mean of all non noDataValues as a
     ...
     */
    protected @Override
    TIntHashSet getModeTIntHashSet() {
    ...
    double getMedianDouble() {
        ...
        return ((Grids_GridInt) getGrid()).getNoDataValue(true);
    }
    ...
    /**
     * Returns a Grids_GridChunkInt64CellMapIterator for iterating over the
     * cells in this.
     *
     * @return
     */
    ...
    Grids_AbstractIterator iterator() {
        return new Grids_GridChunkInt64CellMapIterator(this);
    }
