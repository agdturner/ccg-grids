...
public class Grids_GridInt extends Grids_GridNumber {
     ...
     * @param ge
     */
    protected Grids_GridInt(Generic_Path dir, Generic_Path gridFile,
    ...
    protected Grids_GridInt(Grids_StatsInt stats, Generic_Path dir,
            ...
            int ndv, Grids_Environment ge) throws IOException, ClassNotFoundException {
        super(ge, dir);
        checkDir();
     ...
     * @param stats The Grids_StatsInt to accompany this.
     * @param dir The directory for this.
     * @param g The Grids_GridNumber from which this is to be
 constructed.
    ...
    private void init(Generic_Path file, ObjectInputStream ois) throws IOException, ClassNotFoundException {
        ...
        try {
            ...
            Iterator<Grids_Chunk> chunkIterator;
            chunkIterator = chunkIDChunkMap.values().iterator();
            while (chunkIterator.hasNext()) {
                Grids_Chunk chunk = chunkIterator.next();
                chunk.setGrid(this);
            }
            ...
            try {
                ...
                Grids_GridFactoryDouble gdf = new Grids_GridFactoryDouble(env,
                        gp.GridChunkDoubleFactory,
                        gp.DefaultGridChunkDoubleFactory, -Double.MAX_VALUE,
                        ChunkNRows, ChunkNCols, null,
                        new Grids_StatsNotUpdatedDouble(env));
                Generic_Path ddir = new Generic_Path(Generic_IO.createNewFile(
                        env.files.getGeneratedGridDoubleDir()));
                Grids_GridDouble gd = gdf.create(ddir, file, ois);
                Grids_GridFactoryInt gif;
                gif = new Grids_GridFactoryInt(env, gp.GridChunkIntFactory,
                        gp.defaultChunkIntFactory, Integer.MIN_VALUE,
                        gd.getChunkNRows(), gd.getChunkNCols(), null,
                        new Grids_StatsNotUpdatedInt(env));
                Grids_GridInt gi = (Grids_GridInt) gif.create(dir, gd);
                init(gi);
                // delete gd
                Files.delete(ddir.getPath());
        ...
        // Set the reference to this in the grid stats
        if (getStats() == null) {
            stats = new Grids_StatsNotUpdatedInt(env);
        }
        stats.setGrid(this);
        init();
    }
     ...
     * @param dir The directory for this.
     * @param cf The Grids_ChunkFactoryInt preferred for creating
 chunks.
     * @param chunkNRows The number of rows of cells in any chunk.
     * @param chunkNCols The number of columns of cells in any chunk.
     * @param nRows The number of rows of cells.
     ...
     * @param ndv The ndv.
     */
    private void init(Grids_StatsInt stats, Generic_Path dir,
            Grids_ChunkFactoryInt cf, int chunkNRows,
            int chunkNCols, long nRows, long nCols, Grids_Dimensions dim,
            int ndv) throws IOException {
        env.checkAndMaybeFreeMemory();
        this.stats = stats;
     ...
     * extension containing the data and information about the Grids_GridInt to
     * be returned.
     * @param cf The Grids_ChunkFactoryInt preferred to construct
 chunks of this.
    ...
    private void init(Grids_StatsInt stats, Generic_Path gridFile,
        ...
        if (Files.isDirectory(gridFile)) {
            if (true) {
                Grids_Processor gp = env.getProcessor();
                Grids_GridFactoryInt gf = new Grids_GridFactoryInt(env, gp.GridChunkIntFactory, cf,
                        ndv, chunkNRows, chunkNCols, null, stats);
                Generic_Path thisFile = new Generic_Path(getPathThisFile(gridFile));
                ObjectInputStream ois;
                ois = Generic_IO.getObjectInputStream(thisFile);
                Grids_GridInt g = gf.create(dir, thisFile, ois);
                Grids_GridInt g2 = gf.create(dir, g, startRow, startCol, endRow, endCol);
                init(g2);
            }
        } else {
...
//                Grids_ChunkIntSinglet gridChunk;
    ...
    private void init(Grids_StatsInt stats, Generic_Path gridFile) throws IOException, ClassNotFoundException {
        ...
        if (Files.isDirectory(gridFile)) {
            if (true) {
                Grids_GridFactoryInt gf;
                gf = new Grids_GridFactoryInt(env, gp.GridChunkIntFactory,
                        gp.defaultChunkIntFactory,
                        gp.GridIntFactory.NoDataValue,
                        gp.GridIntFactory.getChunkNRows(),
                        gp.GridIntFactory.getChunkNCols(), null, stats);
                Generic_Path thisFile = new Generic_Path(getPathThisFile(gridFile));
...
//    }
     ...
     */
    @Override
    public Grids_ChunkInt getChunk(
     ...
     * @return
     */
    public int getCell(long row, long col) throws IOException, ClassNotFoundException {
...
//        return ndv;
    ...
    public final int getCell(
            double x,
            double y) throws IOException, ClassNotFoundException {
        long row = getRow(y);
        long col = getCol(x);
        boolean isInGrid = isInGrid(row, col);
        if (isInGrid) {
     ...
     */
    public final int getCell(
            Grids_2D_ID_long cellID) throws IOException, ClassNotFoundException {
...
//        Grids_ChunkInt chunk = getChunk(chunkID);
     ...
     */
    @Override
    public double getNearestValueDouble(double x, double y) throws IOException,
            ClassNotFoundException {
     ...
     */
    @Override
    public double getNearestValueDouble(long row, long col) throws IOException, ClassNotFoundException {
     ...
     */
    public double getNearestValueDoubleDistance(long row, long col) throws IOException, ClassNotFoundException {
        double r = getCell(row, col);
        if (r == NoDataValue) {
     ...
     */
    @Override
    public double getNearestValueDoubleDistance(double x, double y, long row, long col) throws IOException, ClassNotFoundException {
     ...
     * @param value
     */
    protected void initCells(int value) throws IOException, ClassNotFoundException {
 ...
 this.
     */
    public Grids_GridIteratorInt iterator() throws IOException, ClassNotFoundException {
